...
When npm apps are started from the interactive run dashboard, AbpDevTools sets `CI=true` for the npm process unless you already configured `CI`. This keeps dev-server stdin shortcuts, such as Vite's `h + enter`, from stealing AbpDevTools dashboard keys like `L`, `R`, and `S`.
