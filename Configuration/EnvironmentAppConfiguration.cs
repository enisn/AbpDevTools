...
public class EnvironmentToolOption
{
    ...
    public static Dictionary<string, EnvironmentToolOption> GetDefaults()
    {
        ...
        {
            { "sqlserver", new EnvironmentToolOption("docker run --name tmp-sqlserver --restart unless-stopped -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD={Password}' -p 1433:1433 -d mcr.microsoft.com/mssql/server:2017-CU8-ubuntu", "docker kill tmp-sqlserver;docker rm tmp-sqlserver") },
            { "sqlserver-edge", new EnvironmentToolOption("docker run --name tmp-sqlserver --restart unless-stopped -d --cap-add SYS_PTRACE -e 'ACCEPT_EULA=1' -e 'MSSQL_SA_PASSWORD={Password}' -p 1433:1433 mcr.microsoft.com/azure-sql-edge", "docker kill tmp-sqlserver;docker rm tmp-sqlserver") },
            { "postgresql", new EnvironmentToolOption("docker run --name tmp-postgres --restart unless-stopped -e POSTGRES_PASSWORD={Password} -p 5432:5432 -d postgres", "docker kill tmp-posgres;docker rm tmp-posgres") },
            { "mysql", new  EnvironmentToolOption("docker run --name tmp-mysql --restart unless-stopped -e 'MYSQL_ROOT_PASSWORD={Password}' -p 3306:3306 --platform linux/x86_64 -d mysql:5.7", "docker kill tmp-mysql;docker rm tmp-mysql" )},
